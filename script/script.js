...
async function getcards() {
    ...
    div.innerHTML = text;
    div.querySelectorAll("li").forEach((e) => {
        if(!(e.querySelector("a").href.includes(".haccess"))){

            folderlist.push(e.querySelector("a").href);
        }
    });
    folderlist.shift();
    ...
    await folderlist.forEach(async element => {
        ...
        let folderName = element.split("/songs/")[1];
